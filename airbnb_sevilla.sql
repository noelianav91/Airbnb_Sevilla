...
# Número total de airbnb
SELECT DISTINCT id, COUNT(id) AS npropietarios FROM airbnb_sevilla GROUP BY id ORDER BY npropietarios;

#Realizamos una agrupación por precio medio y neighbourhood_group de forma descendente 
SELECT DISTINCT neighbourhood_group, AVG (price) AS avg_precio FROM airbnb_sevilla  GROUP BY neighbourhood_group ORDER BY avg_precio DESC;

#Realizamos una agrupación por neighbourhood y precio
SELECT DISTINCT neighbourhood, neighbourhood_group, AVG (price) OVER (PARTITION BY neighbourhood) AS avg_precio FROM airbnb_sevilla ORDER BY avg_precio DESC;
...
SELECT DISTINCT neighbourhood, neighbourhood_group, COUNT(id) AS numero_Airbnb  FROM airbnb_sevilla GROUP BY neighbourhood, neighbourhood_group ORDER BY numero_Airbnb DESC;
...
SELECT DISTINCT neighbourhood, neighbourhood_group, COUNT(id) AS numero_Airbnb  FROM airbnb_sevilla GROUP BY neighbourhood, neighbourhood_group ORDER BY numero_Airbnb DESC LIMIT 10;
...
SELECT DISTINCT neighbourhood_group, AVG (availability_365) AS availability FROM airbnb_sevilla GROUP BY  neighbourhood_group ORDER BY availability DESC;
